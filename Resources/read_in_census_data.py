...
class CensusInfo(object):
    ...
    file_types = ["5% file", "1% file"]
    five_percent_file = file_types[0]
    one_percent_file = file_types[1]
    record_types = ["Housing Unit Record", "Person Record"]
    household_record = record_types[0]
    person_record = record_types[1]
    record_type_codes = ["H", "P"]
    record_type_dict = dict(list(zip(record_types, record_type_codes)))
    # where the variables names are in the excel sheet
    # DB dictionary
    (variable_field, variable_col) = ("VARIABLE", 5)
    key = "serial_no"
    ...
    backward_syns = defaultdict(list)
    for (k, v) in synonyms.items():
        backward_syns[v].append(k)

    #  Functional properties of the variable, e.g., its start end location in the record,
    #  its explanation, one of each oer variable
    variable_properties_list = ["RT", "BEG", "LEN", "DESCRIPTION"]
    variable_properties_cols = [0, 1, 3, 6]
    variable_properties_dict = dict(
        list(zip(variable_properties_list, variable_properties_cols))
    )

    #  List or description of all the values the variable can have.
    variable_explanation_list = ["LO", "HI", "VALUE DESCRIPTION"]
    variable_explanation_cols_dict = {
        five_percent_file: [7, 8, 9],
        one_percent_file: [10, 11, 12],
    }

    col_names = variable_properties_list + variable_explanation_list + [variable_field]

    for file_type in list(variable_explanation_cols_dict.keys()):
        variable_explanation_cols_dict[file_type] = dict(
            list(
                zip(
                    variable_explanation_list, variable_explanation_cols_dict[file_type]
                )
            )
        )
    ...
    def __init__(self, data_model_file):
            ...
            for row_num in range(thesheet.nrows):
                ...
                if this_var and any((x.strip() for x in row)):
                    ...
                    if this_var not in this_data_dict:
                        ...
                        for (field, col_num) in self.variable_properties_dict.items():
                            val = row[col_num]
                            if field in ["BEG", "LEN"]:
                                try:
                                    val = int(val)
                    ...
                    for file_type in self.file_types:
                        ...
                        if (this_var, file_type) not in this_explanation_dict:
                            ...
                            this_explanation_dict[
                                (this_var, file_type)
                            ] = this_variable_explanation_dict
                        else:
                            this_variable_explanation_dict = this_explanation_dict[
                                (this_var, file_type)
                            ]
                        (low_str, hi_str, desc) = (
                            row[these_cols["LO"]],
                            row[these_cols["HI"]],
                            row[these_cols["VALUE DESCRIPTION"]],
                        )
                        if hi_str:
        ...
        for (rt, var_dict) in self.data_dictionary.items():
    ...
    def print_var_info(self, var0, record_type, file_type=None):
        ...
        (var_info, var_values_info) = (
            self.data_dictionary[record_type][new_var0],
            self.variable_values_dictionary[record_type][(new_var0, file_type)],
        )
        ...
        var_values = sorted(
            var_values_info.keys(), key=lambda x: "".join([str(y) for y in x])
        )
    ...
    def open_db_file(self, db_file, file_type):
        assert (
            file_type in self.file_types
        ), "%s not one of recognized file types %s" % (file_type, self.file_types)
    ...
    def check_specs(self, specs, record_type):
        ...
        for spec in specs:
            ...
            assert not (
                ((op == operator.gt) or (op == operator.lt)) and type(val) == str
            ), (
                "Can only use inequality comparisons with numbers [not strings ('%s')]"
                % (val,)
            )
    ...
    def check_record_type_field(self, record_type, field):
        assert (
            record_type in self.record_types
        ), "%s is not one of the known record types %s" % (
            record_type,
            self.record_types,
        )
        try:
            assert (
                record_type,
                field,
            ) in self.legal_vars, "%s is not a known variable for a %s" % (
                field,
                record_type,
            )
            ...
            try:
                ...
                assert (
                    record_type,
                    new_field,
                ) in self.legal_vars, "%s=>%s is not a known variable for a %s" % (
                    field,
                    new_field,
                    record_type,
                )
    ...
    def select_rows(
        self, specs, record_type, db_file, file_type, op=operator.eq, verbose=False
    ):
        """
        ...
        with self.open_db_file(db_file, file_type) as fh:
            ...
            for line in fh:
                ...
                if self.check_row_record_type(record_type, line):
                    ...
                    if all(
                        (
                            self._cmp_db_val_(line, field, record_type, val, op)
                            for (field, val, op) in new_specs
                        )
                    ):
                        res.append(line)
        if verbose:
    ...
    def print_result_summary(self, specs, res, record_type):
        ...
        max_width = max(list(map(len, spec_strs)))
        print(
            "% *s  %.4f%% of the %ss"
            % (
                max_width,
                spec_strs[0],
                100 * (float(len(res)) / self.event_counter["rows"]),
                record_type,
            )
        )
    ...
    def __get_spec_print_strs__(self, specs):
        ...
        fields_width = max(list(map(len, [ss[0] for ss in specs])))
        ops_width = max(list(map(len, [ss[2].__name__ for ss in specs])))
        vals_width = max(list(map(len, [str(ss[1]) for ss in specs])))
        return [
            "%*s  %*s  %*s"
            % (fields_width, field, ops_width, op.__name__, vals_width, val)
            for (field, val, op) in specs
        ]
...
class DataFrameWrapper(object):
    ...
    def fill_frame(self, query_specs, record_type, db_file, file_type, *col_specs):
        ...
        db_extract = self.data_model.select_rows(
            query_specs, record_type, db_file, file_type
        )
    ...
    def make_complete_frame(self, record_type, db_file, file_type):
        ...
        col_names = list(self.data_model.data_dictionary[record_type].keys())
        return self.fill_frame([], record_type, db_file, file_type, *col_names)
    ...
    def add_key_to_col_specs(self, col_specs):
        ...
        if (key not in col_specs) and (nice_key not in col_specs):
            ...
            return [nice_key] + col_specs[:key_index] + col_specs[key_index + 1 :]
    ...
    def save_frame(self, filename, header=True, separator=","):
        ...
        with open(filename, "w") as ofh:
            for row in rows:
                ...
                print(separator.join(row), file=ofh)
    ...
    def make_subtable(self, cols, filename=None):
        ...
        *** Not yet implemented ***
        ...
        internal_cols = [self.data_model.get_internal_field_name(col) for col in cols]
        ...
        self.col_dict = dict(list(zip(cols, list(range(len(cols))))))

        return subtable

    def panda_save_subtable(self, subtable, filename, header=None, index=False):
        """
        subtable IS a pandas data frame.  This is just a wrapper for
        the pandas method.
        """

        subtable.to_csv(filename, header=header, index=index)
...
if __name__ == "__main__":
    ...
    five_percent_data_file = os.path.join(
        five_percent_dir, "revisedpums5_Alabama_01.txt"
    )
